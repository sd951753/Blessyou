...
async def user_console_manager(connection: Connection):
    while True:
        if connection.client and connection.connected:
            ...
            '''
            #if(flag):
            received_str = await connection.client.read_gatt_char(read_characteristic)
            #print("Received:{0}".format("".join(map(chr,received_str))));
            #print("Received:" + received_str.decode());
            res = int.from_bytes(received_str, byteorder='big', signed=False)
            
            if res == 1: 
                print("Bless you!")
                playsound.playsound('/Users/susietan/Documents/Masters/2020-2021/Fall2020/ECENGR202A/BlessYouSoundEffect.mp3', True)
                time.sleep(0.3)
                res = 0
                received_str = ""
            #await asyncio.sleep(2.0, loop=loop)

            #print(len(received_str.decode()));
              #flag = False
